% This function is for the starting demand for the city
%% Inputs
clear all, close all
CSAstr = 'Raleigh';
% Control Parameters
in.dodisp = true; % display information
% Design Parameters
in.qmin = 10000; % number of people that a grocery store serves
in.v = 15; % Avg vehicle speed (mph)
in.fmax = 96; % Max ld/hr at DC
% think something for f lower bound, say infeasible if fmax is lower
% 96=4*60/(2*(5+10)*5/60), 5*10 mod/array, 5 s/mod, 4 docks, 2 mag fac
in.rwk = 2;  % Avg pkg/wk/person
in.k = 7; % pkg/ld local
% Derived Constants
in.r = in.rwk/(7*24); % Pkg/per-hr per person at DC
in.qmax = in.fmax*in.k/in.r; % Max local pop.
in.qmaxADP = in.qmax;
in.tmax = tmax(CSAstr,in);
if in.dodisp, sdisp(in); end
%% Calculating for 2030
[X30,A30,Aw30] = HDLN2030(CSAstr,in);
%% Select census block points around metro region
[XP,q,aP,zADP] = getADPforCSA20(CSAstr,in); % this could either be from 2020 or 2010 data points
q = round(q*0.2)+1; % decreasing the demand to 20% of the population
...
while ~done
    ...
    [X,F,out1,X30new] = locateDCnf2(fP,XP,aP,X30,in); % Calculating the DC locations using this function
